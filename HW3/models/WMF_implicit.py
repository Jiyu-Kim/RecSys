...
class WMF_implicit():
    def __init__(self, train, valid, n_features=20, reg_lambda =0.1, num_epochs = 550, alpha = 0.5):
        self.train = train
        self.valid = valid
        ...
        for i, row in enumerate(self.train):
            ...
            self.y[i, np.where(row < 0.5)[0]] = self.alpha
    ...
    def fit(self):
        for epoch in range(self.num_epcohs):
            ...
            if epoch % 1 == 0:
                ...
                top_k=50
                print("[WMF] epoch %d, loss: %f"%(epoch, loss))
                prec, recall, ndcg, mrr, mAP = eval_implicit(self, self.train, self.valid, top_k)
                print(f"(WMF VALID) prec@{top_k} {prec}, recall@{top_k} {recall}, ndcg@{top_k} {ndcg}, mrr@{top_k} {mrr}, map@{top_k} {mAP}")
